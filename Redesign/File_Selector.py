...
class FileSelectorForConversion(DPGStage):
    ...
    def generate_id(self,**kwargs):
        ...
        #
        confirmationLabel = kwargs.get("confirmationLabel",f"Convert .{self.inputType.upper()} to .{self.outputType.upper()}")
        #
        ...
        with dpg.window(height=self.height,width=self.width) as self._id:
            ...
            dpg.add_separator()
            #-----------------
            self.next = dpg.add_button(width=500,  label=confirmationLabel, callback=self.nextStage,enabled=False)
    ...
    def openDialogue(self,sender,app_data,user_data):
        with dpg.file_dialog(modal=True,default_path=default_settings_path,callback=self.updateDialogue,height=400,width=400) as self.dialog:
            ...
            dpg.add_file_extension(f".{self.inputType}", color=self.fileColor, custom_text=self.inputType.upper())
